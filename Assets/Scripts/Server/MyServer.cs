...
public class MyServer : MonoBehaviourPun
{
    ...
    IEnumerator WaitForLevel(Player player)
    {
        ...
        if (player.NickName == "") player.NickName = "Player " + (player.ActorNumber - 1);
    ...
    void RPC_CreateBrush(Player playerRequested,Vector2 startPos ,Vector2 endPos)
    {
        ...
        if (_dictModels.ContainsKey(playerRequested))
        {
           ...
           GameManager.Instance.photonView.RPC("RPC_SetDrawingFaces", RpcTarget.All, playerRequested.NickName, true);
           GameManager.Instance.photonView.RPC("RPC_SetThinkingFaces", RpcTarget.All, playerRequested.NickName, false);
    ...
    [PunRPC]
    void RPC_EndDrawAction(Player playerRequested)
    {
        if (!Equals(GameManager.Instance.Turn, playerRequested)) return;
        
        if (_dictModels.ContainsKey(playerRequested))
        { 
            GameManager.Instance.photonView.RPC("RPC_SetDrawingFaces", RpcTarget.All, playerRequested.NickName, false);
            GameManager.Instance.photonView.RPC("RPC_SetThinkingFaces", RpcTarget.All, playerRequested.NickName, true);
        }
